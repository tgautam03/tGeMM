...
void init_mat(MatrixFP16 mat, half val)
{
    // Getting Matrix Dimension
    int n_rows = mat.n_rows; 
    int n_cols = mat.n_cols;
...
void random_init_mat(MatrixFP16 mat, int MAX_VAL, int MIN_VAL)
{
    // Getting Matrix Dimension
    int n_rows = mat.n_rows; 
    int n_cols = mat.n_cols;
    ...
    for (int i = 0; i < n_rows; i++)
    {
        for (int j = 0; j < n_cols; j++)
        {
            mat.ptr[i*n_cols + j] = (half)(rand() % (MAX_VAL - MIN_VAL + 1) + MIN_VAL);
        }
    }
}
